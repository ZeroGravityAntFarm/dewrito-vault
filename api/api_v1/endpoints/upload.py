...
CHUNK_DIR = Path("/app/static/mods/chunks")
CHUNK_SIZE_MAX = 11 * 1024 * 1024  # 11 MB — slight buffer over the 10 MB client chunk size
UPLOAD_ID_RE = re.compile(r'^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$')
...
def upload(background_tasks: BackgroundTasks, modDescription: str = Form(" "), modTags: str = Form(...), modVisibility: bool = Form(...), modGameVersion: str = Form(...), files: List[UploadFile] = File(...), db: Session = Depends(get_db), user: str = Depends(get_current_user)):
    mod_images = []
    valid_files = [".pak"]
    ...
    mod_create, webhooks = controller.create_user_mod(db, modTags=modTags, mod=modData, user_id=user.id, modUserDescription=modDescription, modVisibility=modVisibility, gameVersion=modGameVersion)
...
def _validate_chunk_params(upload_id: str, chunk_index: int, total_chunks: int):
...
def _write_chunk(upload_id: str, chunk_index: int, chunk: UploadFile) -> Path:
    chunk_dir = CHUNK_DIR / upload_id
    chunk_dir.mkdir(parents=True, exist_ok=True)
    chunk_path = chunk_dir / str(chunk_index)
    with open(chunk_path, "wb") as f:
        shutil.copyfileobj(chunk.file, f, 128 * 1024)
    chunk.file.close()
    return chunk_dir


def _assemble_chunks(chunk_dir: Path, total_chunks: int) -> bytes:
    assembled_path = chunk_dir / "assembled"
    with open(assembled_path, "wb") as out:
        for i in range(total_chunks):
            with open(chunk_dir / str(i), "rb") as part:
                shutil.copyfileobj(part, out, 128 * 1024)
...
# Chunked mod upload — new upload
@router.post("/upload/mod/chunk")
def upload_mod_chunk(
    ...
    _validate_chunk_params(upload_id, chunk_index, total_chunks)
    chunk_dir = _write_chunk(upload_id, chunk_index, chunk)

    received = len([p for p in chunk_dir.iterdir() if p.name.isdigit()])
    if received < total_chunks:
        return {"status": "chunk_received", "received": received, "total": total_chunks}

    # All chunks present — assemble and process
    ...
    mod_create, webhooks = controller.create_user_mod(
        db, modTags=modTags, mod=modData, user_id=user.id,
        modUserDescription=modDescription, modVisibility=modVisibility, gameVersion=modGameVersion,
    )
...
# Chunked mod upload — replace existing file
@router.post("/update/mod/{mod_id}/chunk")
def update_mod_chunk(
    mod_id: int,
    upload_id: str = Form(...),
    chunk_index: int = Form(...),
    total_chunks: int = Form(...),
    ...
    # Validate ownership before assembling
    mod_meta = controller.validate_user_mod_file(db, user_id=user.id, mod_id=mod_id)
    if not mod_meta:
        shutil.rmtree(chunk_dir)
