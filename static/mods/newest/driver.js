    ...
    xhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            for (let [i, object] of data["items"].entries()) {
                ...
                trHTML += '</div>';
                trHTML += '<p class="card-text p-3">' + object['modDescription'].substring(0,75) + '...</p>';
                trHTML += '</ul>';
                ...
                trHTML += '</div>';
                trHTML += '<div class="card-footer"><small class="text-muted bi-person-down"> ' + object['mod_downloads'] + '</small><button type="button" class="btn btn-outline-secondary float-end">' + object["modTags"] + '</button></div>';
                trHTML += '</div>';
                trHTML += '</div>';
