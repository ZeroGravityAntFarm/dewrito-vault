    ...
    xhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            for (let [i, object] of data["items"].entries()) {
                ...
                trHTML += '<div class="card text-white bg-dark h-100 rounded-3" >';
                ...
                trHTML += '<a href="/api_v2/maps/' + object['id'] + '/variant/file" class="btn btn-primary me-1">Variant File</a>';
                trHTML += '</div>';
                trHTML += '<div class="card-footer"><small class="text-muted bi-person-down"> ' + object['map_downloads'] + '</small><button type="button" class="btn btn-outline-secondary float-end">' + object["mapTags"] + '</button></div>';
                trHTML += '</div>';
                trHTML += '</div>';
            }
...
function loadCardsv2(page = 1) {
    ...
    const xhttp = new XMLHttpRequest();
    xhttp.open("GET", "/api_v2/maps/?page=" + page + "&size=20&version=" + versionParam);
    xhttp.send();
    xhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            var trHTML = '';
            ...
            for (let [i, object] of data["items"].entries()) {
                ...
                trHTML += '<div class="col mb-3 mt-4">';
                trHTML += '<a href="/api_v2/mapview?mapId=' + object['id'] + '" class="card-link"><div class="card bg-dark tintimg text-white">';
                trHTML += '<img src="/maps/tb/' + object['id'] + '/0" class="card-img" height="200px" alt="..." onerror="this.onerror=null;this.src=\'/content/default/forge.jpg\';">';
                trHTML += '<div class="card-img-overlay">';
                trHTML +=  '<h5 class="card-title">' + object['mapName'] + '</h5>';
                //trHTML +=   '<p class="card-text">This is a wider card with supporting text below as a natural lead-in to additional content. This content is a little bit longer.</p>';
                trHTML +=  '<p class="card-text">Author: ' + object['mapAuthor'] + '</p>';
                trHTML += '</div>';
                trHTML += '</div></a>';
                trHTML += '</div>';
            }
            ...
            trHTML += generatePagination(pages, current, "loadCardsv2");
